...
import com.cems.databinding.FragmentMyEventBinding;
...
public class MyEventFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        Call<ServerResponse> data = ApiInstance.getClient().getMyEvents((String) CEMSDataStore.getUserData().getApiKey());
                ...
                if (response.isSuccessful()) {
                    ...
                    if (serverResponse != null) {
                        if (serverResponse.getStatusCode() == 0) {
                            ...
                            binding.myEventsRecyclerView.setLayoutManager(layoutManager);
                            binding.myEventsRecyclerView.setAdapter(adapter);
                        } else {
                            if (progress.isShowing()) {
                                progress.dismiss();
                            }
                            showAlert("Cannot get events", serverResponse.getMessage());
                        }
                    }
                    else {
                        if (progress.isShowing()) {
